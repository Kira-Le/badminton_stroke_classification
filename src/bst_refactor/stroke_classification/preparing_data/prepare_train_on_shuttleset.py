...
def get_shuttle_result(path: Path, v_width, v_height):
    ...
    # TrackNet flags Visibility=0 with X=Y=0 when it can't detect the shuttle.
    # Flag is the source of truth for shuttle_missing; the (0, 0) coords are
    # otherwise indistinguishable from a real top-left detection.
    shuttle_missing = (df["Visibility"].to_numpy() == 0)
    shuttle_camera = df.drop(columns="Visibility").to_numpy().astype(float)
    # shuttle_camera: (t, 2); shuttle_missing: (t,) bool
    return normalize_shuttlecock(shuttle_camera, v_width, v_height), shuttle_missing
...
def pad_and_augment_one_npy_video(
    ...
    :param shuttle_missing: Per-frame mask, shape (t,) bool. True where TrackNet
        emitted Visibility=0. Strided/padded in lockstep with shuttle; pad
        frames carry True. Not routed through bones / interpolate_joints.
    ...
    :return: Tuple of (pose_dict, pos, shuttle, shuttle_missing, video_len)
        where pose_dict maps each requested style name to its (t, 2, K, d)
        array and video_len is the number of real (non-padded) frames.
    """
    joints = joints.astype(np.float32)
    pos = pos.astype(np.float32)
    shuttle = shuttle.astype(np.float32)
    # shuttle_missing stays bool; the model casts on demand inside forward().

    joints, pos, shuttle, shuttle_missing, new_video_len = make_seq_len_same(
        seq_len, joints, pos, shuttle, shuttle_missing
    )
    ...
    return pose_dict, pos, shuttle, shuttle_missing, new_video_len
...
def collate_npy(
    ...
    for i, branch in enumerate(data_branches):
        ...
        shuttle, shuttle_missing = get_shuttle_result(
            path=csv_path,
        ...
        if min_t < len(failed) or min_t < len(shuttle):
            ...
            shuttle_missing = shuttle_missing[:min_t]
            failed = failed[:min_t]
        ...
        shuttle_missing_ls.append(shuttle_missing)
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for joints, pos, shuttle, shuttle_missing in zip(
            joints_ls, pos_ls, shuttle_ls, shuttle_missing_ls
        ):
            tasks.append(
                executor.submit(
        ...
        for task in tasks:
            pose_dict, pos, shuttle, shuttle_missing, v_len = task.result()
    ...
    shuttle_missing = np.stack(shuttle_missing_ls)
    ...
    np.save(str(set_dir / "shuttle_missing.npy"), shuttle_missing)
