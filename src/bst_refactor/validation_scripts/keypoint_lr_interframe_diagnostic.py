...
Inputs (read from environment):
  - ``BST_MMPOSE_NPY_DIR``: flat dir holding ``{stem}_joints.npy`` per stem.
...
``BST_MMPOSE_NPY_DIR`` path (engelbart for the canonical clean dir).
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to the flat MMPose npy dir. Defaults to BST_MMPOSE_NPY_DIR env.",
    )
    parser.add_argument(
        "--clips-csv",
        type=Path,
    ...
    if npy_dir is None:
        env_npy = os.environ.get("BST_MMPOSE_NPY_DIR")
        if not env_npy:
            raise SystemExit(
                "BST_MMPOSE_NPY_DIR env var not set and --npy-dir not provided."
            )
