...
`DataPaths` resolves paths in priority order: constructor arg > environment variable (or `.env` file entry: `BST_CLIPS_DIR`, `BST_X_SHUTTLE_NPY_DIR`, `BST_X_MMPOSE_NPY_DIR`, `BST_X_CLIPS_CSV`) > `pipeline.config` defaults. Copy `.env.example` at the repo root to `.env` to pin paths per environment.
