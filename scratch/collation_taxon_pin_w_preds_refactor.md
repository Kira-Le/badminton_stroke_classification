...
grep -n "derive_active_classes_from_labels" src/bst_x/stroke_classification/bst_x_common.py
grep -n "derive_active_classes_from_labels\|_derive_active_classes_from_loaded_labels" src/bst_x/stroke_classification/bst_x_train.py
...
grep -n "_validate_and_record_arch" src/bst_x/stroke_classification/bst_x_train.py
...
grep -n "taxonomy_info = f._.self.taxonomy.name" src/bst_x/stroke_classification/bst_x_train.py
# Expected: line ~803.
...
find src/bst_x/stroke_classification/experiments \
...
New file: `src/bst_x/stroke_classification/collation_runner.py`. Thin loop over a session config, one cell per (taxonomy, split, collation_id, n_serials), plus an optional training `ablation_id` per cell. No kill rules, no verdict logic. State persisted to `state.json` for resume.
...
- `src/bst_x/stroke_classification/bst_x_train.py` (Hyp tuple, train_network return value, dump_predictions, asserts, _validate_and_record_arch deletion, CLI flags)
- `src/bst_x/stroke_classification/bst_x_infer.py` (--fe + --fe-output-dir, batch dump path)
- `src/bst_x/stroke_classification/bst_x_common.py` (delete derive_active_classes_from_labels, add dump_topk_predictions)
- `src/bst_x/stroke_classification/collation_runner.py` (new)
...
- `src/bst_x/run_tracker.py` *or* `src/bst_x/stroke_classification/bst_x_train.py` (one line landing `config.classes` into the manifest payload per Step D1.5; recommended in bst_x_train at the `track_run` call site)
...
- `src/bst_x/stroke_classification/hparam_sweep.py` (kill/verdict logic operates on metrics; needs minor audit for CLI flag shape compatibility — see Step F3 — but the core loop is unaffected)
