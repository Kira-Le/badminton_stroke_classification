...
| `src/bst_refactor/stroke_classification/main_on_shuttleset/bst_x_common.py` | Add `derive_active_classes_from_labels(taxonomy, train_labels, validation_label_arrays)`. Replaces first-fix's `remap_and_validate_labels`. Per §5.2. |
...
| `src/bst_refactor/stroke_classification/main_on_shuttleset/bst_x_infer.py` | Strip 1 — required kwargs `n_active_classes` and `active_class_list`, no fallback. Update `__main__` example to read both from manifest. |
